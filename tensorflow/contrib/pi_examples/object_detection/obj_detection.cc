...
ObjectDetection::Impl::Impl(const Parameters& params)
  ...
  // Read labels
  tensorflow::Status read_labels_status = ReadLabelsFile(labels, &str_labels);
...
tensorflow::Status ObjectDetection::Impl::ReadEntireFile(
...
tensorflow::Status ObjectDetection::Impl::ReadTensorFromImageFile(
...
// can use to run it.
tensorflow::Status ObjectDetection::Impl::LoadGraph(
...
std::vector<std::pair<std::string, float>> ObjectDetection::Impl::Detect(
  ...
  // LOG(INFO) << "size:" << outputs.size() << ",image_width:" << imgWidth <<
  // ",image_height:" << imgHeight << endl;
